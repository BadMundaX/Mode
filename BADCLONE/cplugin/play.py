...
from BADCLONE.cplugin.buttons import (
    livestream_markup,
    playlist_markup,
    slider_markup,
    track_markup,
...
from BADCLONE.utils.inline import close_markup, aq_markup
...
from BADCLONE.utils.logger import play_logs, clone_bot_logs
from BADCLONE.cplugin.setinfo import get_logging_status, get_log_channel
from BADCLONE.cplugin.utils.cthumbnail import get_thumb
...
async def stream(client, _, mystic, user_id, result, chat_id, user_name, original_chat_id, video: Union[bool, str] = None, streamtype: Union[bool, str] = None, spotify: Union[bool, str] = None, forceplay: Union[bool, str] = None, userbot=None):
    ...
    if streamtype == "playlist":
        ...
        for search in result:
            ...
            if await is_active_chat(chat_id):
                ...
                try:
                    ...
                    continue
                await Bad.join_call(chat_id, original_chat_id, file_path, video=status, image=thumbnail, bot_id=bot_id, client=client)
        ...
        if await is_active_chat(chat_id):
            ...
            if not forceplay:
                db[chat_id] = []
            await Bad.join_call(chat_id, original_chat_id, file_path, video=None, bot_id=bot_id, client=client)
        ...
        if await is_active_chat(chat_id):
            ...
            if not forceplay:
                db[chat_id] = []
            await Bad.join_call(chat_id, original_chat_id, file_path, video=status, bot_id=bot_id, client=client)
        ...
        if await is_active_chat(chat_id):
            ...
            await Bad.join_call(chat_id, original_chat_id, file_path, video=status, image=thumbnail if thumbnail else None, bot_id=bot_id, client=client)
        ...
        if await is_active_chat(chat_id):
            ...
            if not forceplay:
                db[chat_id] = []
            await Bad.join_call(chat_id, original_chat_id, link, video=True if video else None, bot_id=bot_id, client=client)
