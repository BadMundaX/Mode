...
async def seek_comm(cli, message: Message, _, chat_id):
    if len(message.command) == 1:
        return await rich_reply(message, _["admin_20"])
    query = message.text.split(None, 1)[1].strip()
    if not query.isnumeric():
        return await rich_reply(message, _["admin_21"])
    playing = db.get(chat_id)
    if not playing:
        return await rich_reply(message, _["queue_2"])
    duration_seconds = int(playing[0]["seconds"])
    if duration_seconds == 0:
        return await rich_reply(message, _["admin_22"])
    ...
    if message.command[0][-2] == "c":
        ...
        to_seek = duration_played + duration_to_skip + 1
    mystic = await rich_reply(message, _["admin_24"])
    ...
    try:
        ...
        return await rich_edit(mystic, _["admin_26"], reply_markup=close_markup(_))
    if message.command[0][-2] == "c":
