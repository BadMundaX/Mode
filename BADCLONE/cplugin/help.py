...
def clone_help_markup(_, page: int, total: int = 5):
    """Next/Back pager for the clone-bot help pages (CLONE_HELP_2..6),
    which had to be split into pages because the combined text blew
    past Telegram's 1024-char media caption limit (this help panel is
    a photo caption, not a plain text message)."""
    nav = []
    if page > 1:
        nav.append(
            InlineKeyboardButton(
...
def first_page(_):
    controll_button = [
        ...
        InlineKeyboardButton(text="๏ ɴᴇxᴛ ๏", callback_data=f"BadxBaby"),
    ]
