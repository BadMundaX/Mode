    ...
    if audio_telegram:
        ...
        if await JioSaavn.valid(url):
            if await JioSaavn.valid_playlist(url):
                ...
                cap = _["play_10"].format(app.mention, message.from_user.mention)
...
async def play_music(client, CallbackQuery, _):
    ...
    try:
        await stream(_, mystic, CallbackQuery.from_user.id, details, chat_id, user_name, CallbackQuery.message.chat.id, video, streamtype="jiosaavn", forceplay=ffplay)
    except Exception as e:
        ex_type = type(e).__name__
        err = e if ex_type == "AssistantErr" else _["general_2"].format(ex_type)
        return await mystic.edit_text(err)
...
async def play_playlists_command(client, CallbackQuery, _):
    ...
    try:
        await stream(_, mystic, user_id, result, chat_id, user_name, CallbackQuery.message.chat.id, video, streamtype=stream_type, forceplay=ffplay)
    except Exception as e:
        ex_type = type(e).__name__
        err = e if ex_type == "AssistantErr" else _["general_2"].format(ex_type)
        return await mystic.edit_text(err)
