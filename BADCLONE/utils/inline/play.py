...
from BADCLONE.utils.database import get_autoplay_cached, get_loop_cached
...
def _timer_row(chat_id, played, dur):
    """Progress bar button: 00:47 ┃┊—♡——————┊┃ 4:06 (colour follows the progress)."""
...
def _control_rows(_, chat_id):
    """Main player buttons (button texts are plain on purpose, restyle them as you like)."""
    return [
        [
...
def _more_rows(_, chat_id):
    ...
    repeat_on = get_loop_cached(chat_id) != 0
    autoplay_on = get_autoplay_cached(chat_id)
    thumb_on = get_thumbnail_status(chat_id) == "on"

    return [
        [
            InlineKeyboardButton(text="🔀 Shuffle", callback_data=f"PLAYERSHUFFLE|{chat_id}", style=random_style()),
            InlineKeyboardButton(
                text="🔁 Repeat ✅" if repeat_on else "🔁 Repeat",
                callback_data=f"PLAYERREPEAT|{chat_id}",
                style=random_style(),
            ),
            InlineKeyboardButton(text="📥 Video", callback_data=f"downloadvideo {vidid}", style=random_style()),
            InlineKeyboardButton(text="📥 Audio", callback_data=f"downloadaudio {vidid}", style=random_style()),
        ],
        [
            InlineKeyboardButton(
                text="Autoplay | On" if autoplay_on else "Autoplay | Off",
                callback_data=f"autoplay_from_player|{chat_id}",
                style=ButtonStyle.SUCCESS if autoplay_on else ButtonStyle.DANGER,
            ),
            InlineKeyboardButton(
                text="Thumbnail | On" if thumb_on else "Thumbnail | Off",
                callback_data=f"THUMBTOGGLE|{chat_id}",
                style=ButtonStyle.SUCCESS if thumb_on else ButtonStyle.DANGER,
            ),
        ],
        [InlineKeyboardButton(text="Back", callback_data=f"PLAYERBACK|{chat_id}", style=random_style())],
    ]


def stream_markup_timer(_, chat_id, played, dur):
    return [_timer_row(chat_id, played, dur)] + _control_rows(_, chat_id)


def stream_markup(_, chat_id):
    return _control_rows(_, chat_id)


def more_markup_timer(_, chat_id, played, dur):
    return [_timer_row(chat_id, played, dur)] + _more_rows(_, chat_id)


def more_markup(_, chat_id):
    return _more_rows(_, chat_id)
...
def playlist_markup(_, videoid, user_id, ptype, channel, fplay):
    buttons = [
        [
            InlineKeyboardButton(
                text=_["P_B_1"],
                callback_data=f"BadPlaylists {videoid}|{user_id}|{ptype}|a|{channel}|{fplay}", style=random_style(),
            ),
            InlineKeyboardButton(
                text=_["P_B_2"],
                callback_data=f"BadPlaylists {videoid}|{user_id}|{ptype}|v|{channel}|{fplay}", style=random_style(),
            ),
        ],
        [
            InlineKeyboardButton(
